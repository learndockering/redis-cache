...
public enum KvCacheType {
	...
	REDIS("redis", DeployType.SINGLETON, com.keifer.core.cache.redis.RedisCache.class), REDIS_CLUSTER("redis",
			DeployType.CLUSTER, com.keifer.core.cache.redis.cluster.ClusterRedisCache.class), REDIS_SHARED("redis",
					DeployType.SHARED, com.keifer.core.cache.redis.sharded.ShardedRedisCache.class);
	...
	public static void check(KvCacheType ktype, Class<?> clazz) {
		if (ktype.getTargetClazz() != clazz) {
			throw new BeanInitializationException("expected class: " + clazz.getName() + ",but not found.");
		}
	}
